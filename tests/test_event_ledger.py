...
class EventLedgerTests(unittest.TestCase):
    ...
    def test_ten_required_scenarios(self):
        ...
        scenarios = {
            "submit_no_response": ([submitted], snapshot("applied")),
            "ack_screen_rejection": (
                [
                    submitted,
                    event("acknowledgement_received", 2),
                    event(
                        "interview_scheduled",
                        3,
            ...
            "assessment_technical_offer": (
                [
                    submitted,
                    event("assessment_invited", 2, occurred_at=NEXT_DAY, payload={"assessment_id": "test-1"}),
                    event(
                        "assessment_completed", 3, occurred_at=NEXT_DAY, payload={"assessment_id": "test-1"}
                    ),
                    event(
                        "interview_scheduled",
                        4,
            ...
            "wrong_interview_voided_after_rejection": (
                [
                    ...
                    event("event_voided", 4, occurred_at=None, precision="unknown", supersedes="event-002"),
                ],
                snapshot("rejected", response=NEXT_DAY),
            ),
            "listing_closes_after_submit": ([submitted], snapshot("applied")),
            ...
            "hypothetical_mail_no_event": ([submitted], snapshot("applied")),
        }
    ...
    def test_date_precision_and_belgrade_boundary(self):
        rows = [event("application_submitted", 1, occurred_at="2026-09-22T23:30:00Z", precision="instant")]
    ...
    def test_correction_replaces_type_in_original_slot(self):
        rows = [
            ...
            event("response_received", 3, occurred_at=NEXT_DAY, supersedes="event-002"),
        ]
        self.assertEqual(ledger.project_events(rows), snapshot("applied", response=NEXT_DAY))
        self.assertEqual(
            [item["event_id"] for item in ledger.effective_events(rows)], ["event-001", "event-003"]
        )
    ...
    def test_rejects_contract_and_correction_errors(self):
        ...
        self.assert_invalid(
            [
                submitted,
                event("event_voided", 2, occurred_at=None, precision="unknown", supersedes="event-999"),
            ],
            "bad_supersedes",
        )
        self.assert_invalid(
            [
                submitted,
                event("response_received", 2),
                event("event_voided", 3, occurred_at=None, precision="unknown", supersedes="event-002"),
                event("event_voided", 4, occurred_at=None, precision="unknown", supersedes="event-002"),
            ],
            "forked_correction",
        )
        self.assert_invalid([submitted, event("interview_scheduled", 2, payload={})], "bad_payload")
        self.assert_invalid(
            [
                submitted,
                event("interview_cancelled", 2, payload={"round_id": "missing-1", "round_kind": "recruiter"}),
            ],
            "bad_transition",
        )
        self.assert_invalid(
            [submitted, event("rejection_received", 2), event("offer_received", 3)], "bad_transition"
        )
        self.assert_invalid(
            [
                submitted,
                event("offer_received", 2),
                event("interview_scheduled", 3, payload={"round_id": "late-1", "round_kind": "technical"}),
            ],
            "bad_transition",
        )
        with self.assertRaises(ledger.EventValidationError) as caught:
            ledger.project_events(
                [submitted, event("response_received", 2, job_id="job-9002")], job_ids={JOB_ID, "job-9002"}
            )
    ...
    def test_parse_and_mismatch_report_are_read_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assert_invalid(
                [event("application_submitted", 1, precision="unknown", occurred_at=None)], "bad_precision"
            )
    ...
    def test_cross_file_duplicate_event_id_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(ledger.EventValidationError) as caught:
                ledger.mismatch_report(root, {JOB_ID: snapshot("applied"), "job-9002": snapshot("applied")})
    ...
    def test_cli_reports_mismatch_without_mutation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "scripts" / "event_ledger.py"),
                    str(event_dir),
                    "--jobs-csv",
                    str(csv_path),
                ],
