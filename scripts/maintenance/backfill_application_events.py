...
if __package__:
    from scripts.event_ledger import (  # noqa: E402
        EventValidationError, compare_snapshot, mismatch_report, project_events,
    )
...
def plan(root, recorded_at):
    ...
    for row in rows:
        job_id = row["id"]
        category = (row["application_status"], row["stage_reached"], bool(row["applied_at"]), bool(row["response_at"]))
    ...
    return {
        "rows": len(rows), "untouched_jobs": untouched, "existing_jobs": len(existing), "existing_events": existing_count,
        "pending_jobs": len(pending), "pending_events": sum(counts.values()),
        "event_counts": {kind: counts[kind] for kind in MIGRATION_TYPES},
        "blocked": blocked, "categories": dict(sorted(categories.items())),
...
def main(argv=None):
    ...
    try:
        ...
        if args.format == "json":
            ...
            print(f"{summary['outcome']}: {summary['rows']} rows, {summary['pending_jobs']} pending jobs, "
