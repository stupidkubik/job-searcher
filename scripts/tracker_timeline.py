...
def event_views(events):
    ...
    for event in events:
        ...
        state = "superseded" if successor else (
            "void_marker" if event["event_type"] == "event_voided" else "effective"
        )
        views.append({
            "event_id": event_id,
...
def read_timeline(root, job_id):
    ...
    with locked(root):
        ...
        if events:
            ...
            if differences:
                raise EventValidationError("snapshot_mismatch", f"event history disagrees with jobs.csv: {differences}")
        return {
            "job_id": job_id,
            "company": row["company"],
            "role": row["role"],
            "history_state": "event_history" if events else (
                "legacy_snapshot_only" if row["applied_at"] else "no_application_events"
            ),
