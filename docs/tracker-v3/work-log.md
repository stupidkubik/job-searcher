...
| L-0018 | 2026-09-24 | `codex/tracker-v3` / WP1.6 timeline change | Добавлена read-only `jobs.py timeline job-NNNN` с evidence, precision, correction state и следующим действием из snapshot; production history не мигрирована | `tracker_timeline.py`, CLI/документация, synthetic legacy/migration/correction/void/mismatch tests | 268 tests green; strict validation 458/474; tracker/index fresh; live `job-0001` = `legacy_snapshot_only`; `git diff --check` | verified |
| L-0019 | 2026-09-24 | `codex/tracker-v3` / Gate 1 cutover safety checkpoint | D-016 закрывает snapshot-only `status` после event history или при включённом gate; B-006 фиксирует нерешённую совместимость, production writes выключены | `tracker_write.py`, status/connector negative tests, `gate-1-checkpoint-2026-09-24.md`; branch после fetch 0 behind `origin/main` | 272 tests green; strict validation 458/474; migration dry-run 45 jobs/53 events/0 blocked; tracker/index fresh; `git diff --check` | verified; Gate 1 open |
| L-0020 | 2026-09-24 | `codex/tracker-v3` / D-017 cutover implementation | Выбран explicit `event` для lifecycle и confirmed `set` для `cv_version`; versioned marker готов к атомарной публикации вместе с migration; canonical events ещё не записаны | `tracker_paths.py`, `agent_operations.py`, migration/rehearsal, contract, card front matter, tests; temp copy rollback exact and 45 jobs/53 events, retry 0 pending | 277 tests green; strict validation 458/474; contract/tracker/index fresh; `git diff --check` | verified; migration pending |
| L-0021 | 2026-09-24 | `codex/tracker-v3` / D-017 canonical migration commit | После fresh fetch (0 behind main) опубликованы 45 event files / 53 date-only migration events и marker одним journal; обновлены рабочие инструкции, CSV и generated views не менялись | `data/application_events/*`, `config/event-ledger-cutover.json`, `cutover-2026-09-24.md`; post-apply retry 0 pending, 0 snapshot mismatches | 277 tests green; strict validation 458/474; contract/tracker/index fresh; marker enables writes without env | verified on branch; merge pending |
| L-0022 | 2026-09-24 | `main` / PR #21, then `codex/v3-defer-packets` | Подтверждён merge Gate 1; пользователь отложил полный packet workflow для личного трекера | D-019 supersedes D-004; Q-006/Q-007 deferred; Phase 3/4 independent of Gate 2; план, матрица, риски и cutover status обновлены | PR checks pass; merged tree equals checked branch; 11 documentation tests green; strict validation 458/474; tracker/index/contract fresh; `git diff --check` | verified |
