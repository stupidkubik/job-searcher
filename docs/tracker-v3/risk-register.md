...
| R-002 | Event ledger and snapshot diverge during dual-write or process crash | medium | critical | validator finds different status/stage/dates after a replace boundary | one transaction service; durable recovery protocol; crash/fault injection; no independent writers; consistency gate | write path | active |
