...
| R-002 | Event ledger and snapshot diverge during dual-write or process crash | medium | critical | validator finds different status/stage/dates after a replace boundary | shared writer/reader lock; durable recovery; result in same boundary; integrated crash/fault tests and consistency gate | write path | active |
| R-003 | Correction mutates or obscures historical events | medium | high | old event disappears, order changes or incorrect stage survives correction | append-only supersession; loop validation; recompute effective stage; render fact and projection | event model | active |
...
| R-016 | Legacy status creates snapshot-only lifecycle after event cutover | high | critical | event history and CSV diverge or a new application has no event | D-016 pre-write fence; B-006 compatibility policy and cutover tests | cutover | active |
| R-007 | Packet manifest points to changed files | high | high | hash mismatch after CV edit | if D-019 is revisited, raw-byte SHA-256; immutable packet version; fail closed; clear stale report | packet | deferred |
| R-008 | Packet/log leaks personal data or secrets | medium | high | CI/result contains document content, token or email body | if D-019 is revisited, paths/hashes only in machine result; redaction; local secret storage; fixtures synthetic | privacy | deferred |
...
| R-010 | Agent variability changes score/recommendation | high | high | materially similar evidence yields unexplained score shifts | versioned score and rationale; explicit eligibility/unknown; golden corpus and review, no silent rescore | matching | active |
...
| R-015 | Repository/bootstrap weight grows sharply | high | medium | normal agent boot reads event/match corpus | compact indexes; targeted commands; measure bytes/tokens every phase | ergonomics | active |
...
| R-017 | Scope expands into SPA/DB/browser before core contracts stabilize | high | high | new runtime/dependency without measured need | D-008 and D-019; phase gates; explicit new decision required | scope | active |
