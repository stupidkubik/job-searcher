...
> ✅ Исправлено 2026-08-16, см. [«M5 — Wellfound/getmatch: политика уже
> написана, данные приведены в соответствие»](#m5--wellfoundgetmatch-политика-уже-написана-данные-приведены-в-соответствие)
> в конце документа.
...
> ✅ Разобрано 2026-08-16, см. [«M6 — все оставшиеся пары проверены, дублей
> нет»](#m6--все-оставшиеся-пары-проверены-дублей-нет) в конце документа.
> После M4 осталось 8 пар (14 были порождены заглушками агрегаторов); ни одна
> из 8 не оказалась настоящим дублем.
...
> ✅ Разобрано 2026-08-16, см. [«M7 — не баг, задокументировано»](#m7--не-баг-задокументировано)
> в конце документа. `job-0164`/`job-0168` — на самом деле разные вакансии
> (M6); расхождение сигналов — намеренное и теперь описано в
...
> ✅ Разобрано 2026-08-16, см. [«M8 — все запросы без результата проверены;
> найден и восстановлен 8-й, активно сломанный»](#m8--все-запросы-без-результата-проверены-найден-и-восстановлен-8-й-активно-сломанный)
> в конце документа.
...
| ~~8~~ | ~~L1–L8~~ | ✅ сделано |
| 9 | M9–M12 | гигиена (данные и код, не тронуты в этом проходе) |
...
## M5 — Wellfound/getmatch: политика уже написана, данные приведены в соответствие

Рекомендация просила «решить и записать», но при чтении
[`docs/sources/wellfound.md`](docs/sources/wellfound.md) выяснилось, что
решение уже задокументировано — строка 17: «native card/form не является
first-party employer listing», строка 39: `original_url` должен быть
external redirect на реальный employer/ATS, а не сам Wellfound card; строка
41: если такого редиректа нет, `original_url` должен оставаться пустым, а
verification — `unknown`. Вопрос был не политикой, а несоответствием данных
уже существующему правилу.

Уточнено у пользователя (не додумано): проверка по всем 7 записям
(`job-0065`, `job-0066`, `job-0067`, `job-0068`, `job-0069`, `job-0130` →
Wellfound; `job-0198` → `getmatch.ru`) фактически ограничивалась native
listing агрегатора — реальная employer/ATS страница не открывалась.
`job-0198` был особым случаем: notes утверждали «Getmatch is the employer and
the exact vacancy is hosted on its own platform» — то есть предыдущая сессия
приняла агрегатора за работодателя, ровно ту ошибку, которую фиксирует M4.

Для всех 7: `original_url` очищен, `first_party_verified` и `apply_verified`
сброшены в `unknown` через `set <id> original_url= first_party_verified=unknown
apply_verified=unknown notes=...` — один вызов на запись, `notes` дополнены
пояснением о переклассификации (не заменены, чтобы не потерять исходный
контекст проверки). `verified_at` при этом обновился на `2026-08-16`
автоматически (поле защищено — `set` не даёт обнулить его отдельно) — это
корректно: сегодня запись действительно была пересмотрена, и итог этого
пересмотра — `unknown`, а не то, что проверки не было вовсе.

Побочный эффект: `job-0065`–`job-0069` не появились в `verification_queue`
(M1) — у них уже стоит `decision_reason` (`salary_too_low`,
`role_not_frontend`), то есть они не активные кандидаты и повторная
first-party проверка для них не нужна. `job-0130` (applied) и раньше не
проверялся по-настоящему — теперь корректно появляется в
`verification_queue`. `job-0198` не появляется — `listing_status=closed`.

Проверено: `validate --strict` — 0/0, `render-tracker --check` — актуален,
122 теста зелёные, `dupes` не изменился (8 пар, оба поля не участвуют в
similarity).

### Тесты

Отдельных тестов не добавлено — правка данных через существующий,
уже покрытый write-path `set`; поведение `verification_queue` для
активных/неактивных записей уже проверено тестом из M1.

122 теста, весь набор зелёный (без изменений — тестов не добавлялось).

---

## M6 — все оставшиеся пары проверены, дублей нет

После фикса M4 (уникальные суффиксы для 21 записи с агрегатором вместо
работодателя) `dupes` упал с 22 пар до 8 — 14 были ложными совпадениями
исключительно на строке-заглушке `company`. Оставшиеся 8 разобраны вручную,
по `notes`/`source_url`/полям строки, без изменения данных:

| Пара | Компания | Почему не дубль |
|---|---|---|
| `job-0009`/`job-0010` | Bavarian Capital | разные роли (Intern vs Graduate) |
| `job-0030`/`job-0164` | Undisclosed/Hirify | разные Hirify ID, независимо screened на разные причины |
| `job-0046`/`job-0047` | ClickUp | `notes` называют разные страны (Болгария vs Чехия) — country-specific постинги |
| `job-0063`/`job-0186` | FetchJobs.co | разные LinkedIn ID, разные `decision_reason`, репост агрегатором разных вакансий |
| `job-0100`/`job-0101` | FCamara | разная специализация (Angular vs generic Frontend) |
| `job-0103`/`job-0106` | Mindrift | `notes`: Бразилия vs Саудовская Аравия — разные страновые постинги |
| `job-0112`/`job-0114` | Pavago | `notes`: Пакистан vs Филиппины — разные страновые постинги |
| `job-0194`/`job-0195` | Яндекс MultiTrack | разные продукты (Игры vs Директ), разные vacancy ID |

Все 8 — ложные срабатывания эвристики `similarity(company, role)`: она не
видит гео/специализацию, только текст компании и роли. Ни одна пара не
требует `add --duplicate-of`, `--force` или screening — это не «неразобранные
дубли», это уже разобранные не-дубли.

**Отклонение от рекомендации аудита.** Рекомендация предлагала «разобрать
пары, затем снять `continue-on-error`». При разборе выяснилось, что для уже
существующих строк в CLI нет пути отметить пару как «проверено, не дубль» —
`decision_reason=duplicate_listing` создаётся только через `add
--duplicate-of` в момент добавления (`scripts/jobs.py:1764`), а не задним
числом на две уже существующие записи. Компании в этом датасете регулярно
постят одну роль в несколько стран (см. таблицу выше) — это будет
воспроизводить fuzzy-пары бесконечно, независимо от разбора. Снять
`continue-on-error` без allowlist-механизма означало бы получить точно тот
же класс бага, что чинил C1: heuristic-сигнал без входа для «уже
рассмотрено», намертво стоящий в CI.

Решение (подтверждено пользователем): оставить `continue-on-error: true` в
`.github/workflows/validate.yml` как есть — это осознанный design trade-off
эвристики без allowlist, а не забытый флаг. Добавление allowlist-механизма
(новый формат данных, CLI-флаг, тесты) — отдельная работа, если и когда
понадобится реально закрыть `--fail`; не сделано в рамках этого пункта.
Данные и код не менялись.

### Тесты

Не требуются — анализ существующих данных, без изменений в коде или CSV.

122 теста, весь набор зелёный (без изменений).

---

## M7 — не баг, задокументировано

Перепроверка снимает часть исходного описания. `job-0164` и `job-0168` — это
не «одна и та же роль с двумя разными исходами»: у них разные Hirify ID (это
и есть та самая пара из M6 — уже проверена и признана не дублем), разный
`level` (`Senior` против `Middle`) и полностью разные основания в `notes`:
у `job-0164` — hard seniority blocker (5+ лет), у `job-0168` — archived/stale
листинг без first-party route. Это два независимых, корректных решения по
двум разным вакансиям, а не расхождение по одному входу.

Собственно структурная часть находки — что `decision_reason` при ручном/
connector screening не обязан совпадать с тем, что выдал бы keyword-based
`TOO_SENIOR_SIGNALS` из `scripts/ingestion.py` (используется только в
batch `ingest`) — подтвердилась. Проверено по всем 27 записям с
`decision_reason=seniority_too_high` в датасете: каждая либо (а) содержит
explicit `Senior`/`Staff` в title — то, что поймал бы и keyword-фильтр, либо
(б) называет в `notes` конкретное требование к опыту (`3+ года`, `5+ лет` и
т.п.), которого в title нет и которое `ingest`-фильтр в принципе не может
увидеть, потому что не читает тело вакансии. Включая `job-0159` (`Middle
React Developer`, `seniority_too_high`, `notes`: «requires 3+ years of
commercial React experience») — это не противоречит `config/profile.md`
(«Junior/Middle — только как stretch»): stretch обусловлен фактическим
требуемым опытом, а не title, и `config/profile.md` отдельно уточняет, что
реальный коммерческий product-frontend опыт кандидата меньше заявленных ~3
лет общего опыта — так что 3+ года именно коммерческого React обоснованно
превышают порог.

**Выбор рекомендации.** Сводить ручной screening к тем же сигналам, что и
`ingest` (вариант 1), было бы регрессией: это выбросило бы весь
years-of-experience evidence из notes и заставило бы принимать решения хуже,
чем сейчас. Выбран вариант 2 — задокументировано в
[`docs/sources/README.md`](docs/sources/README.md) (новый абзац в разделе
«5. Screening и полный анализ»): `ingest`-фильтр — грубый title-only
pre-filter для batch-пути, ручной/connector screening законно использует
более богатый evidence из полного текста вакансии, расхождение по
одинаковому `role`/`level` не является багом само по себе, если у записей
разные source ID и оно объяснено в `notes` — сравнивать нужно по evidence, а
не по строке `role`.

Данные и код не менялись — только `docs/sources/README.md`.

### Тесты

Не требуются — документация и повторная проверка существующих данных, без
изменений в коде или CSV.

122 теста, весь набор зелёный (без изменений).

---

## M8 — все запросы без результата проверены; найден и восстановлен 8-й, активно сломанный

### Новая находка: `hirify-pass-20260816` активно сломан, 9 вакансий потеряны

При проверке `data/operations/` обнаружился восьмой запрос без результата,
которого не было в исходном аудите — `hirify-pass-20260816`, создан сегодня
в 03:03. Это atomic batch на 9 новых `add` (Busyboard, MacPaw, Monetha,
nove8, Addi, Kolesa Group, YouHodler, Welcome to the Jungle, V4Scale).
`gh run view` по соответствующему workflow run показал реальную причину:

```
error: unknown add args: next_action
```

`next_action` не входит ни в `jobs.ADD_INPUT_FIELDS` (`scripts/jobs.py:70`),
ни в `ADD_ALLOWED_ARGS` коннектора (`scripts/agent_operations.py:37`) — оно
устанавливается после создания записи через `set`/`status`, не при `add`.
Несколько child-операций в batch содержали это поле; из-за `atomic: true`
упал весь batch, ни одна из 9 вакансий не попала в `jobs.csv`, результат не
записан. Это не «заброшенный» запрос из исходного аудита — это активно
сломанная, свежая операция с реальными потерянными данными.

С подтверждения пользователя восстановлено: все 9 добавлены через
`jobs.py add --stdin` (те же данные из `args`, без `next_action`), затем
`next_action` для 6 из них доставлен отдельным `set <id> next_action=...` —
он не защищён и полностью settable после создания. Получили `job-0215`…
`job-0223`. `validate --strict` — 0/0, `render-tracker --check` — актуален,
122 теста зелёные. Сам запрос-файл (`data/operations/requests/hirify-pass-20260816.json`)
не редактировался — исходный (пусть и невалидный) запрос остаётся неизменной
исторической записью того, что реально было запрошено.

### Исходные 7 — все проверены, все уже разрешены другим путём

Для каждого из 7 запросов из исходного аудита канонические данные сверены с
намерением запроса:

| Запрос | Намерение | Canonical состояние сейчас |
|---|---|---|
| `add-hirify-web-developer-823409-20260813` | добавить Hirify 823409 | `job-0150`, тот же `source_url` — уже существует |
| `add-serpapi-junior-fullstack-20260812` | добавить SerpApi Junior Fullstack | `job-0144`, тот же HN `source_url` — уже существует |
| `wttj-zen-educate-emea-product-20260812` | добавить Zen Educate Product Engineer | `job-0138` — уже существует |
| `board-cleanup-0054-0022-0023-0129-20260812` | status job-0054; screen job-0022/0023/0129 | все 4 уже в целевом состоянии (`applied`; `decision_reason=other` × 3) |
| `job-updates-20260813-1153` | status job-0144/0148/0053; screen job-0149 | все 4 уже в целевом состоянии (`applied` × 2, `rejected`, `decision_reason=other`) |
| `status-0055-0130-20260812` | status job-0055/job-0130 | оба уже в целевом состоянии (`rejected`, `applied`) |
| `verify-watchlist-six-20260812` | verify job-0065/66/67/68/69/128 | все 6 уже закрыты через `screen` с конкретным `decision_reason` (`salary_too_low`/`role_not_frontend`/`geo_restriction`) вместо запланированного `verify` — другой, но эквивалентный по результату путь разрешения |

Все 7 подтверждены как **не потерянная работа**: canonical данные полностью
отражают намерение каждого запроса, просто через другую операцию/сессию, а
не через этот конкретный request. Ни одному не требуется восстановление.

### Почему result-файлы не созданы

`data/operations/README.md`: «The runner creates the matching result exactly
once» — это гарантия, что `results/` содержит только то, что реально
обработал доверенный GitHub Actions runner. Локальная сессия (эта) не
проходит через тот пайплайн; написать туда `status=abandoned`/`conflict`
вручную означало бы подделать запись, которая по контракту удостоверяет
«CI это обработал» — то есть подорвать ровно ту гарантию audit trail, которую
защищает этот файл.

**Решение (подтверждено пользователем):** только документация здесь, без
изменений `data/operations/`. Код (например, отдельная `mark-abandoned`
команда, явно помечающая результат как ручной override, не CI) не добавлен —
это отдельная работа, если понадобится реально закрыть каталог для будущих
агентов, которые иначе будут заново тратить время на разбор этих же 8
файлов.

### Тесты

Не требуются для документационной части (7 старых запросов). Для
восстановленных 9 вакансий — отдельных тестов не добавлено: это данные,
записанные через уже покрытый write-path `jobs.py add`/`set`.
...
## L1–L8 — все низкие закрыты одним проходом

Все 8 findings — документация, схема и два узких кода-фикса; ни один не
требовал ручной правки canonical CSV.

- **L1.** `AGENTS.md:123` пример `next_action="follow up"` заменён на
  `next_action="follow-up"` — теперь совпадает с подстрокой, которую ищет
  `todo_sections` (`scripts/jobs.py:2728`, ищет `follow-up` через дефис) и с
  формой, уже используемой в данных.
- **L2.** `docs/roadmap.md:13` — убрано устаревшее «review/direct delivery
  modes» (review PR удалён коммитами `f9909ec`/`995ea35`), заменено на
  «direct-to-`main` delivery without operation branches or review PRs» —
  формулировка синхронизирована с `docs/agent-operations.md` и
  `docs/current-architecture.md`.
- **L3.** Уже был закрыт как побочный эффект M2 — `README.md:47` и
  `docs/jobs-cli.md:269` оба используют `--cv-version frontend-2026-08`.
  Отдельных правок не потребовалось.
- **L4.** `data/schema.md:6` противоречил строке 31 (английский-only vs
  «как в первоисточнике»). Строка 6 явно исключает `company`/`role` — ровно
  то же ограничение, что уже задокументировано в таблице полей, без
  расширения на `location` (там нет реальных non-ASCII значений в данных, в
  отличие от `company`/`role`, где есть `Т-Банк`, `Яндекс MultiTrack`).
- **L5.** `norm_url` (`scripts/jobs.py:373`) больше не вырезает `source` из
  query-параметров при дедупликации URL — только `ref`/`referrer`/`utm_*`,
  которые всегда чистый tracking-шум. `source` может быть значимым
  ATS-идентификатором вакансии; на затронутых 0 URL в текущих данных это
  ничего не меняет, но закрывает будущий false-positive дубль.
- **L6.** `--format json` добавлен для `migrate-v2`, `backfill-sources` и
  `report` — это были единственные subcommands без него.
  `migrate_summary_payload` — новая функция, отделяющая структурированную
  сводку от text-рендеринга (`migration_summary` теперь строится поверх неё,
  а не наоборот). `backfill-sources --format json` оборачивает тот же
  `summary`-dict, который уже использовал text-режим. `report --format json`
  печатает тот же `stats_payload`, что и `stats --format json` — они и раньше
  строились из одной функции, просто `report` не открывал json-выход.
  `docs/jobs-cli.md:72` обновлён: вместо перечисления команд — «все
  subcommands `jobs.py`», так как ограничение больше не существует.
- **L7.** Добавлен пустой `tests/__init__.py` — `python3 -m unittest discover
  -s tests -t .` теперь работает наравне с формой без `-t`, которую использует
  CI.
- **L8.** `collect_records` (`scripts/import_himalayas.py`) получил
  `MAX_PAGES_PER_PASS = 50`: при аномальном ответе API (большой `totalCount`,
  пустые `jobs`) цикл паджинации теперь падает с явной `HimalayasImportError`
  вместо того, чтобы полагаться только на `timeout-minutes: 15` в workflow.

### Тесты

Отдельных новых тестов не добавлено — все восемь фиксов либо документационные
(L1–L4), либо покрыты существующими тестами по конструкции (`norm_url`,
`--format json` роутинг, `unittest discover`), либо defensive guard без
достижимого в текущих данных сценария (L8, как и было в M2/H-фиксах для
похожих latent-risk случаев). Вручную проверено: `backfill-sources --check
--format json`, `report --format json`, `migrate-v2 --format json` через
`install_v1_fixture()`, `unittest discover -s tests -t .` и `-s tests`
(эквивалент CI) — оба зелёные.

122 теста, весь набор зелёный (без изменений в самих тестах).
